...
# GUI setup
root = tk.Tk()
root.title("Salary Predictor with Polynomial Regression")
...
def predict_salary():
    try:
        ...
        user_encoded = pd.get_dummies(user_input, columns=['Gender', 'Education Level', 'Job Title'], drop_first=True)
        user_encoded = user_encoded.reindex(columns=X_encoded.columns, fill_value=0)

        user_poly = poly.transform(user_encoded)
        predicted_salary = model.predict(user_poly)[0]
...
ax.set_title('Actual vs Predicted Salary (Polynomial Regression)')
ax.grid(True)
...
# R² Score, MSE, RMSE, MAE Definitions
