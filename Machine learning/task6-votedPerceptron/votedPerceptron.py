...
# only first 5 features for GUI simplicity
X = X[:, :5]

# Preprocessing
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)
y = np.where(y == 0, -1, 1)  # Convert to -1 and 1 for Voted Perceptron
...
class VotedPerceptron:
    ...
    def predict(self, X):
        predictions = []
        for x in X:
            vote_sum = sum(v * np.sign(np.dot(w, x)) for w, v in zip(self.weights, self.votes))
...
# GUI setup
root = tk.Tk()
root.title("Voted Perceptron Classifier")
